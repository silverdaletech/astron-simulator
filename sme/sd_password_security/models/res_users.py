...
class ResUsers(models.Model):
    ...
    def get_password_policy(self):
        """
        This function is use to get password policy configure in config setting on company level
        """
    ...
    def password_match_message(self):
        """
        This function is use to return validation to user while changing Password to match password requirements
        """
        self.ensure_one()
    ...
    def _check_password_rules(self, password):
        """
        Will check password rules set on res_config and allow to change when password meet all requirements.
        """
        self.ensure_one()
        if not password:
            return True
        ...
        if not re.search("".join(password_regex), password):
            _logger.info('Password does not match the policy')
        ...
        if estimation["score"] < company_id.password_estimate:
            _logger.info('Password score is lower than company requirement')
    ...
    def _password_has_expired(self):
        """
        Will check password expiry date
        """
        self.ensure_one()
        if not self.password_write_date:
            _logger.info('password has expired due to missing write date')
            return True
        ...
        _logger.info('%s > %s ? %s' %(days, self.company_id.password_expiration, days > self.company_id.password_expiration))
        return days > self.company_id.password_expiration

    def action_expire_password(self):
        """
        will force to  password reset if password is expired
        """
        _logger.info('Action to Expire Password')
    ...
    def _validate_pass_reset(self):
        ...
        """
        _logger.info('Validation Password Reset')
        for rec_id in self:
            ...
            if write_date + delta > datetime.now():
                _logger.error('%s + %s > %s ? %s' %(write_date, delta, datetime.now(), write_date + delta > datetime.now()))
                raise UserError(
                    _(
    ...
    def _check_password_history(self, password):
        ...
        """
        _logger.info('Check Password History')
