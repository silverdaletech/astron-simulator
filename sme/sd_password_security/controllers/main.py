...
import logging
_logger = logging.getLogger('Password-Security')
...
class PasswordSecurityHome(AuthSignupHome):
    ...
    def web_login(self, *args, **kw):
        ...
        ## we are commenting above if and adding following two lines code-----
        user = request.env['res.users'].sudo().search([('login', '=', request.params['login'])], limit=1)
        if not user._password_has_expired():
            return response
        # My password is expired, kick me out
        # request.env.user.action_expire_password()
        # commenting above one line and adding following 4 lines of code
        _logger.info('Password has expired, kick out user')
        # user = request.env['res.users'].sudo().search([('login', '=', request.params['login'])], limit=1)
        ## we are commenting ove one line of code here
        if user:
        ...
        if user:
            ...
            redirect = request.env.user.partner_id.signup_url
        _logger.info('Redirecting .. %s' %redirect)
