...
class StockQuant(models.Model):
    ...
    def check_negative_qty(self):
        """
        Check for the negative quantity in Quants by location
        """
        ...
        for quant in self:
            disallowed_by_location = quant.location_id.prevent_negative_stock
            if (
                ...
                raise ValidationError(
                    _(
                        "You cannot complete this inventory movement as it will cause the stock level of the product "
                        "'%(name)s'%(name_lot)s become negative (%(q_quantity)s) on source location '%(complete_name)s'"
                    )
                    % {
