...
class PosPaymentMethod(models.Model):
    """
    Inherit to add fields for Square payment terminals
    """
    ...
    def _get_payment_terminal_selection(self):
        ...
        return super(PosPaymentMethod, self)._get_payment_terminal_selection() + [
            ('sd_pos_square_payment_terminal', 'Square Payment Terminal')]
    ...
    def create(self, vals):
        ...
        if not res.device_code and res.use_payment_terminal == 'sd_pos_square_payment_terminal':
            res.message = 'Please add device code by clicking on Generate Device Code Button to be able to use the ' \
                          'terminal.'
        return res
            ...
            if device_code:
                ...
                if not device_record:
                    device_record = self.env['device.code'].create({
                        ...
                        'pair_by': datetime.strptime(device_code.get('pair_by'), '%Y-%m-%dT%H:%M:%S.%fZ').strftime(
                ...
                view_id = self.env.ref(
                    'sd_pos_square_payment_terminal.custom_response_message_wizard_form').id
                if view_id:
                    value = {
                        'name': _('Response'),
                        'view_type': 'form',
                        'view_mode': 'form',
                        'res_model': 'custom.response.message.wizard',
                        'view_id': False,
        ...
        try:
            ...
            if self.token_key and pos_payment and pos_payment.transaction_id and not pos_payment.refunded_id:
                ...
                body = {
                    ...
                    "payment_id": pos_payment.transaction_id,
...
class PosPayment(models.Model):
    """Interit to add fields for Square payment terminals """
