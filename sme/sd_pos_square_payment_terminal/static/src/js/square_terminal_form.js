odoo.define("sd_pos_square_payment_terminal.payment", function (require) {
  "use strict";
  ...
  var SquarePaymentTerminal = PaymentInterface.extend({
        ...
        debugger
          ...
          debugger
           ...
           device_id = result.device_id
            return self._sd_pos_square_payment_terminal_pay();
          }
          else{
           self._show_error(
              _t(result.message)
    ...
    _sd_pos_square_payment_terminal_pay: function () {
      var self = this;
      var line = this.pos.get_order().selected_paymentline;
      ...
      if (order === this.poll_error_order) {
        ...
        return self._sd_pos_square_payment_terminal_handle_response({});
      }
...
//      });
    ...
    _sd_pos_square_payment_terminal_handle_response: function () {
      if (this.was_cancelled) {
        return Promise.resolve();
      }
      var line = this.pos.get_order().selected_paymentline;
...
//                      clearInterval(self.polling);
          ...
          if (data){
            ...
            self._show_error(
              _t('Payment transaction has been Failed,PLease check the status of Payment on square account.',))
               self._sd_pos_square_payment_terminal_cancel();
               line.set_payment_status("retry");
               resolve(false);
            }
            }
    ...
    _sd_pos_square_payment_terminal_cancel: function () {
      var self = this;
      var data = "";
      var line = this.pos.get_order().selected_paymentline;
    ...
    send_payment_cancel: function (order, cid) {
      ...
      if (payment_id) {
        this._sd_pos_square_payment_terminal_cancel();
      }
